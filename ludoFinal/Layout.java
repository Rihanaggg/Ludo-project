...
// Layout class to draw board
 class Layout {
	...
	public void draw(Graphics2D g){
		g.setColor(Color.LIGHT_GRAY); // Color of background of board
        ...
        for(int i=1;i<6;i++) { // Drawing the Final path
        	g.setColor(Color.RED);
        ...
        g.setColor(Color.RED); //Connecting link between Home box and Final path
    	...
    	for(int i=0;i<2;i++) { // Drawing the home pawn boxes
    		for(int j=0;j<2;j++) {
    			g.setColor(Color.RED);
    	...
    	for(int i=0;i<3;i++) { // Drawing the outer path boxes
    		for(int j=0;j<6;j++) {
    	...
    	g.drawRect(x+((1)*width),y+(1*height),4*width,4*height); //inner line of home box 
        ...
        for(int i=0;i<2;i++) { 
    		for(int j=0;j<2;j++) {   //inner square of home
